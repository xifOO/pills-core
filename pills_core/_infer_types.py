...
class TypeInferencer:
    ...
    def _detect_type(self, series: pd.Series) -> Literal["numeric", "categorical", "datetime", "unknown"]:
        if series.empty:
            return "unknown"
    ...
    def _inspect_object_column(self, series: pd.Series) -> Literal["numeric", "categorical", "datetime", "unknown"]:
        clean = series.dropna()
        sample = clean.sample(n=min(len(clean), self.max_sample_size), random_state=42) if len(clean) > 0 else pd.Series([], dtype=object)
