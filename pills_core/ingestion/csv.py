        ...
        if pd.api.types.is_string_dtype(series):
            ...
            clean_num = pd.to_numeric(
                temp_col.str.replace(r"[$,%\s]", "", regex=True).str.replace(
                    self.options.decimal, ".", regex=False
                ) if self.options.decimal != "." else temp_col.str.replace(r"[$,%\s]", "", regex=True),
                errors="coerce",
            )

            if len(series) > 0 and clean_num.notna().sum() > len(series) * 0.5:
                series = clean_num.astype(np.float64)
